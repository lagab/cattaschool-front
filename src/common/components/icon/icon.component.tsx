...
let allIcons: Record<string, React.ReactElement> = {
  Basket,
  Heart,
  User,
  Search,
...
const toCapitalize = (stringToCapitalize: string): string => {
  ...
  stringToCapitalize.split(' ').forEach((value) => {
    capitalizedString += `${value.toLowerCase().charAt(0).toUpperCase()}${value
      .toLowerCase()
      .slice(1)} `;
  });
...
const Icon: React.FC<IconProps> = ({ classes, iconType, ...props }) => {
  if (iconType) {
    const IconSvgr = allIcons[
      toCapitalize(iconType)
    ] as unknown as () => React.ReactElement<IconProps>;
    const additionalClasses = s[iconType];
    return (
      <IconSvgr
        className={classnames(s.icon, additionalClasses, classes)}
        {...props}
      />
    );
  }
